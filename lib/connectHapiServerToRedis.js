...
console.log('REDIS-SESSION-READER RUNNING IN DEBUG/CONSOLE MODE');
    ...
    server.auth.strategy('cookieAuth',  'seelioAuth', {
      authenticate: function(req, cb) {
        ...
        if (!rawSid) {
          ...
          console.log('REDIS-SESSION-READER: Cookie does not exist');
        ...
        if (!sid) {
          ...
          console.log('REDIS-SESSION-READER: Cookie is not properly signed');
          return cb( new Boom.unauthorized('Not authorized'));
          ...
          getSessionIdFromRedis(sid, server.plugins.redisPlugin.client, function (err, userId) {
            if (err) {
              console.log('REDIS-SESSION-READER: Cookie signed, but user not authorized. sid is ' + sid);
              return new Boom.unauthorized('Not authorized');
            }
